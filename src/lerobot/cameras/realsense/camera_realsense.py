...
class RealSenseCamera(Camera):
    ...
    def __init__(self, config: RealSenseCameraConfig):
        ...
        if self.capture_width is None and self.capture_height is None and self.height and self.width:
    ...
    def _postprocess_image(
        ...
        target_color_mode = self.color_mode if color_mode is None else color_mode

        processed_image = image
        if not depth_frame and target_color_mode == ColorMode.BGR:
        ...
        if self.width is not None and self.height is not None:
            ...
            if output_width != self.width or output_height != self.height:
                interpolation = cv2.INTER_NEAREST if depth_frame else cv2.INTER_LINEAR
                processed_image = cv2.resize(
                    processed_image, (self.width, self.height), interpolation=interpolation
                )
