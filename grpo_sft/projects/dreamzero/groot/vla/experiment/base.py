...
class BaseTrainer(transformers.Trainer):
    ...
    def __init__(self, **kwargs):
        ...
        # Extract GRPO config before super().__init__ pops kwargs
        self._grpo_use = kwargs.pop("use_grpo", False)
        self._grpo_weight = kwargs.pop("grpo_weight", 0.1)
        self._grpo_reward_scale = kwargs.pop("grpo_reward_scale", 1.0)
        super().__init__(**kwargs)


        # Initialize GRPO components
        self.use_grpo = self._grpo_use
        self.grpo_weight = self._grpo_weight
        self.grpo_reward_scale = self._grpo_reward_scale
        self.grpo_weight_buffer = None
        if self.use_grpo:
            self.grpo_weight_buffer = GRPOWeightBuffer()
            print("[GRPO] Initialized with LingBot-style action reward reweighting")
        self.loss_queues = {}
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # GRPO: LingBot-style action reward and sliding-window loss reweighting.
        if self.use_grpo:
            try:
                action = inputs.get("action", None)
                if action is None:
                    raise KeyError("GRPO enabled but batch has no 'action' tensor")
                action_mask = inputs.get("action_mask", inputs.get("actions_mask", None))
                reward = compute_action_grpo_reward(
                    action,
                    action_mask,
                    reward_scale=self.grpo_reward_scale,
                )
                if self.grpo_weight_buffer is None:
                    self.grpo_weight_buffer = GRPOWeightBuffer()
                stats = self.grpo_weight_buffer.update(reward)
                grpo_weight = stats["weight"].to(device=loss.device, dtype=loss.dtype)
                loss = loss * grpo_weight
                if self.current_step % 5 == 0:
                    self.log({
                        "grpo_reward": stats["reward"].item(),
                        "grpo_advantage": stats["advantage"].item(),
                        "grpo_weight": stats["weight"].item(),
                    })
            except Exception as e:
                if self.current_step <= 5 or self.current_step % 100 == 0:
                    print(f"[GRPO] Warning: {e}", flush=True)
...
class BaseExperiment(ABC):
    def __init__(self, cfg: DictConfig):
        ...
        print(f"[DEBUG {time.time()}] About to call create_train_dataset", flush=True)
        train_dataset = self.create_train_dataset(cfg, model)
        print(f"[DEBUG {time.time()}] create_train_dataset returned", flush=True)
        print("Using dataset:")
        print(train_dataset)
        assert (
    ...
    def create_model(self, cfg, training_args):
        import time
        print(f"[DEBUG {time.time()}] Entering create_model", flush=True)
        model = instantiate(cfg.model)
        print(f"[DEBUG {time.time()}] Model instantiated", flush=True)

        if cfg.pretrained_model_path is not None:
            mprint(f"Loading pretrained weights from: {cfg.pretrained_model_path}")
            print(f"[DEBUG {time.time()}] About to load pretrained weights from {cfg.pretrained_model_path}", flush=True)
            import json, gc
    ...
    def create_train_dataset(self, cfg, model):
        print(f"[DEBUG {time.time()}] Entering create_train_dataset", flush=True)
        assert torch.distributed.is_initialized()
        print(f"[DEBUG {time.time()}] torch.distributed is initialized", flush=True)
        print(f"[DEBUG {time.time()}] About to instantiate train_dataset", flush=True)
        train_dataset = instantiate(cfg.train_dataset)
        print(f"[DEBUG {time.time()}] train_dataset instantiated successfully", flush=True)
    ...
    def create_trainer(
        ...
        # Inject GRPO config into trainer
        trainer.use_grpo = getattr(cfg, "use_grpo", False)
        trainer.grpo_weight = getattr(cfg, "grpo_weight", 0.1)
        trainer.grpo_reward_scale = getattr(cfg, "grpo_reward_scale", 1.0)
        if trainer.use_grpo:
            trainer.grpo_weight_buffer = GRPOWeightBuffer(
                buffer_size=getattr(cfg, "grpo_buffer_size", 32),
                warmup_steps=getattr(cfg, "grpo_buffer_warmup_steps", 4),
                alpha=getattr(cfg, "grpo_advantage_alpha", getattr(cfg, "grpo_alpha", 1.0)),
                clamp_min=getattr(cfg, "grpo_weight_clamp_min", 0.5),
                clamp_max=getattr(cfg, "grpo_weight_clamp_max", 2.0),
            )
            print(
                "[GRPO] LingBot-style action reward enabled: "
                f"reward_scale={trainer.grpo_reward_scale} "
                f"buffer_size={trainer.grpo_weight_buffer.buffer_size} "
                f"warmup={trainer.grpo_weight_buffer.warmup_steps} "
                f"alpha={trainer.grpo_weight_buffer.alpha} "
                f"clamp=[{trainer.grpo_weight_buffer.clamp_min},"
                f"{trainer.grpo_weight_buffer.clamp_max}]"
            )
