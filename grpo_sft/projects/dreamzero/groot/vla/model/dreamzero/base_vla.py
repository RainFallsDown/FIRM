...
class VLA(PreTrainedModel):
    ...
    def validate_inputs(self, inputs):
        ...
        # Auto-fix video format if needed
        if "video" in inputs:
            video = inputs["video"]
            # Fix shape: [B, T, num_cameras, H, W, C] -> [B, T, num_cameras, C, H, W]
            if isinstance(video, torch.Tensor) and len(video.shape) == 6 and video.shape[-1] == 3 and video.shape[3] != 3:
                # Transpose last 3 dimensions: [H, W, C] -> [C, H, W]
                video = video.permute(0, 1, 2, 5, 3, 4)
                # Don't convert to numpy here - let it stay as Tensor for the model
                # The validation will be skipped since we reset detected_error
                inputs["video"] = video
                detected_error = False  # Reset error flag after fix
