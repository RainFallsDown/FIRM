...
class CausalConv3d(nn.Conv3d):
    ...
    def forward(self, x, cache_x=None):
        ...
        if cache_x is not None and self._padding[4] > 0:
            print(f"[DEBUG VAE] cache_x.shape={cache_x.shape}, x.shape={x.shape}, dim=2", flush=True)
...
class Down_ResidualBlock(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        x_copy = x.clone()
        print(f"[Down_ResidualBlock] Input x.shape: {x.shape}")
        
        # Process through main path
        for module in self.downsamples:
            print(f"[Down_ResidualBlock] Before {module.__class__.__name__}: x.shape={x.shape}")
            x = module(x, feat_cache, feat_idx)
            print(f"[Down_ResidualBlock] After {module.__class__.__name__}: x.shape={x.shape}")
        
        # Process shortcut
        shortcut = self.avg_shortcut(x_copy)
        print(f"[Down_ResidualBlock] Shortcut shape: {shortcut.shape}, Main path shape: {x.shape}")
        
        # Align temporal dimension if mismatch
        if shortcut.shape[2] != x.shape[2]:
            print(f"[Down_ResidualBlock] Temporal dimension mismatch! Aligning shortcut from {shortcut.shape[2]} to {x.shape[2]}")
            # Downsample shortcut temporal dimension to match main path
            shortcut = shortcut[:, :, ::2, :, :]
            print(f"[Down_ResidualBlock] After alignment: shortcut.shape={shortcut.shape}")
...
class Encoder3d_38(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        print(f"[Encoder3d_38] Input x.shape: {x.shape}")
        
        # Initial convolution: 12 channels -> dims[0] (128)
        # CausalConv3d only accepts (x, cache_x), not feat_cache/feat_idx
        x = self.conv1(x)
        print(f"[Encoder3d_38] After conv1: x.shape={x.shape}")
        
        # Downsample blocks
        for module in self.downsamples:
            x = module(x, feat_cache, feat_idx)
        print(f"[Encoder3d_38] After downsamples: x.shape={x.shape}")
        ...
        for module in self.middle:
            if hasattr(module, '__call__'):
                ...
                x = module(x)
        print(f"[Encoder3d_38] After middle: x.shape={x.shape}")
        
        # Output head
        for module in self.head:
            x = module(x)
        print(f"[Encoder3d_38] After head: x.shape={x.shape}")
...
class WanVideoVAE(nn.Module):
    ...
    def encode(self, videos, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        hidden_states = []
        print(f"[DEBUG WanVideoVAE.encode] Processing video {len(hidden_states)+1}/{len(videos)}", flush=True)
        for video in videos:
