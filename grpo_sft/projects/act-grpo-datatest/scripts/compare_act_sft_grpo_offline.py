...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare ACT SFT/BC and GRPO checkpoints through the same offline "
            "training-forward metric style used for LingBot GRPO vs SFT."
        )
    )
    parser.add_argument(
        "--checkpoint",
        action="append",
        required=True,
        help="Checkpoint spec in the form label=/path/to/checkpoint_or_pretrained_model.",
    )
...
def evaluate_checkpoint(
    ...
    payload["dataset_repo_id"] = cfg.dataset.repo_id
    payload["dataset_root"] = str(cfg.dataset.root)
    payload["dataset_num_frames"] = len(dataset)
    return payload, cfg


def build_deltas(results: dict[str, dict[str, object]]) -> dict[str, dict[str, float]]:
    labels = list(results)
    if len(labels) < 2:
        return {}
    baseline = labels[0]
    ...
    for label in labels[1:]:
        ...
        for key in LOWER_IS_BETTER:
            ...
            label_delta[f"{key}_delta"] = value - base_value
            label_delta[f"{key}_ratio"] = value / base_value if base_value else math.nan
            label_delta[f"{key}_relative_change_pct"] = (
                ((value - base_value) / base_value) * 100.0 if base_value else math.nan
            )
        deltas[f"{label}_vs_{baseline}"] = label_delta
    return deltas
...
def write_summary(path: Path, payload: dict[str, object]) -> None:
    ...
    baseline = labels[0]
    lines = [
        "# ACT Haimiandian Offline SFT vs GRPO Evaluation",
        "",
    ...
    for metric in LOWER_IS_BETTER:
        values = [
            f"{float(payload['results'][label]['metrics'][metric]):.8f}"
            for label in labels
        ]
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "- Metrics are computed through the ACT training forward path on normalized LeRobot batches.",
            "- `val_loss` is ACT reconstruction L1 plus `kl_weight * val_kld_loss`.",
            "- `action_*` metrics compare predicted normalized action chunks with target normalized action chunks under `action_is_pad`.",
