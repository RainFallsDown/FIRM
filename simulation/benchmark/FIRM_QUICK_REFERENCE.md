...
source ${CONDA_ROOT:-/path/to/miniconda3}/bin/activate lerobot

# sam3环境（用于SAM3 mask生成）
source ${CONDA_ROOT:-/path/to/miniconda3}/bin/activate sam3
```
...
ssh <remote-host> << 'EOF'
source ${CONDA_ROOT:-/path/to/miniconda3}/bin/activate lerobot
cd ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}

# 1. 准备工作空间
python firm_benchmark/prepare_lerobot_annotations.py \
  --dataset-root ${TIANQING_DATA_ROOT:-/path/to/tianqing}/tianqing_data/test_data_rain/test_data_haimiandian/A2p_dataset_haimiandian_resize_1776393366 \
...
### 数据集
- **原始数据**: `${TIANQING_DATA_ROOT:-/path/to/tianqing}/tianqing_data/test_data_rain/test_data_haimiandian/A2p_dataset_haimiandian_resize_1776393366`
  - `data/chunk-000/`: parquet文件（50个episodes）
  - `videos/chunk-000/`: AV1编码视频（6个相机视角）
  - `meta/`: 元数据

### 工作空间
- **主目录**: `${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/annotation_workspace_sponge/`
...
### 结果
- **报告目录**: `${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/results_sponge/`
...
**解决方案**: 批量更新任务类型
```bash
ssh <remote-host> "cd ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace} && \
...
model = sam3.build_sam3_image_model(
    ...
    checkpoint_path="${SAM3_CHECKPOINT_DIR:-/path/to/sam3}/sam3.pt",
    load_from_HF=False,
)
...
### 3. 运行脚本
```bash
cd ${FIRM_BENCHMARK_ROOT:-/path/to/simulation/benchmark}
UV_CACHE_DIR=/tmp/uv-cache uv --directory ${SAM3_WORKDIR:-/path/to/sam3_workspace} run --no-sync --active \
  python ${FIRM_BENCHMARK_ROOT:-/path/to/simulation/benchmark}/firm_benchmark/generate_sam3_masks_real.py \
  --episodes-root annotation_workspace_sponge/episodes \
  --checkpoint ${SAM3_CHECKPOINT_DIR:-/path/to/sam3}/sam3.pt \
  --sam3-repo ${SAM3_REPO:-/path/to/sam3_repo} \
...
### 4. 验证masks
```bash
ls -la ${FIRM_BENCHMARK_ROOT:-/path/to/simulation/benchmark}/annotation_workspace_sponge/episodes/episode_000000/masks/object/ | head -10
```
...
### 查看单个episode的结构
```bash
ssh <remote-host> "tree ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/annotation_workspace_sponge/episodes/episode_000000 -L 2"
```

### 查看meta.json内容
```bash
ssh <remote-host> "cat ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/annotation_workspace_sponge/episodes/episode_000000/meta.json | python -m json.tool"
```

### 查看评估结果
```bash
ssh <remote-host> "cat ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/results_sponge/report.md"
ssh <remote-host> "cat ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/results_sponge/summary.json | python -m json.tool"
```

### 检查mask文件
```bash
ssh <remote-host> "ls -lh ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/annotation_workspace_sponge/episodes/episode_000000/masks/object/ | head -5"
ssh <remote-host> "file ${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/annotation_workspace_sponge/episodes/episode_000000/masks/object/000000.png"
```
...
- **服务器**: <remote-host> (SSH)
- **项目负责人**: 需要补充
- **SAM3代码**: `${SAM3_REPO:-/path/to/sam3_repo}`
- **SAM3模型**: `${SAM3_CHECKPOINT_DIR:-/path/to/sam3}`
- **FIRM代码**: `${FIRM_BENCH_ROOT:-/path/to/firm-benchmark-workspace}/firm_benchmark/`
