...
## 项目目标
- 在服务器 `<remote-host>` 上完整运行 FIRM benchmark 评估流程。
- 使用真实 SAM3 模型生成 object masks，并输出 Sponge 任务 DAP 指标。

## 关键路径
- 项目路径: `${FIRM_BENCHMARK_ROOT:-/path/to/simulation/benchmark}`
- 本地 SAM3 模型目录: `${SAM3_CHECKPOINT_DIR:-/path/to/sam3}`
- SAM3权重: `${SAM3_CHECKPOINT_DIR:-/path/to/sam3}/sam3.pt`
- SAM3代码路径: `${SAM3_REPO:-/path/to/sam3_repo}`
...
- 已完成（更新）：
  ...
  - 默认即对接本机真实 SAM3 路径：
    - `--checkpoint` 默认 `${SAM3_CHECKPOINT_DIR:-/path/to/sam3}/sam3.pt`
    - `--sam3-repo` 默认 `${SAM3_REPO:-/path/to/sam3_repo}`
