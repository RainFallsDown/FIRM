...
def test_get_prev_chunk_left_over_uses_current_timestep(policy_server):
    """RTC leftovers should be sliced from the current next-action timestep."""
    prev_chunk = torch.arange(8 * 6, dtype=torch.float32).reshape(8, 6)
    policy_server._update_prev_chunk_state(prev_chunk, start_timestep=10)

    left_over = policy_server._get_prev_chunk_left_over(current_timestep=14)

    torch.testing.assert_close(left_over, prev_chunk[4:])


def test_predict_action_chunk_passes_rtc_kwargs(monkeypatch, policy_server):
    """RTC-enabled server inference should pass delay and leftovers to the policy."""
    from lerobot.async_inference.policy_server import PolicyServer
    ...
    policy_server._rtc_latency_tracker.add(0.09)
    prev_chunk = torch.arange(8 * 6, dtype=torch.float32).reshape(8, 6)
    policy_server._update_prev_chunk_state(prev_chunk, start_timestep=5)
    ...
    obs = _make_obs(torch.zeros(6), timestep=8)
    timed_actions = policy_server._predict_action_chunk(obs)

    assert len(timed_actions) == policy_server.actions_per_chunk
    assert captured_kwargs["inference_delay"] == 3
    torch.testing.assert_close(captured_kwargs["prev_chunk_left_over"], prev_chunk[3:])


def test_record_rtc_latency_uses_observation_latency_sample(policy_server):
    """RTC latency tracking should use the latency attached to the queued observation."""
    policy_server.rtc_config = RTCConfig(enabled=True, execution_horizon=4)
    obs = _make_obs(torch.zeros(6), timestep=3)
    obs.one_way_latency_s = 0.2

    policy_server._record_rtc_latency(obs, server_turnaround_s=0.05)

    assert policy_server._rtc_latency_tracker.max() == pytest.approx(0.45)
