...
def test_timed_observation_getters():
    ...
    to = TimedObservation(timestamp=ts, observation=obs_dict, timestep=0, one_way_latency_s=0.125)
    ...
    assert math.isclose(to.get_one_way_latency_s(), 0.125, rel_tol=0, abs_tol=1e-6)
...
def test_timed_data_deserialization_data_getters():
    ...
    assert math.isclose(to_out.get_one_way_latency_s(), 0.25, rel_tol=0, abs_tol=1e-6)
...
def test_latency_tracker_max_uses_current_window():
    """Tracker max should reflect only the current sliding window."""
    tracker = LatencyTracker(maxlen=2)

    for sample in [1.0, 10.0, 2.0, 3.0]:
        tracker.add(sample)

    assert len(tracker) == 2
    assert math.isclose(tracker.max(), 3.0, rel_tol=1e-6)
