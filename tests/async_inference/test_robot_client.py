...
def test_robot_client_embeds_rtc_config_in_remote_policy():
    """Remote policy specs sent to the server should carry RTC settings."""
    from lerobot.async_inference.configs import RobotClientConfig
    from lerobot.async_inference.robot_client import RobotClient
    from tests.mocks.mock_robot import MockRobotConfig
    ...
    config = RobotClientConfig(
        ...
        pretrained_name_or_path="test",
        actions_per_chunk=20,
        rtc=RTCConfig(enabled=True, execution_horizon=7),
    )

    client = RobotClient(config)
    try:
        assert client.policy_config.rtc.enabled is True
        assert client.policy_config.rtc.execution_horizon == 7
    finally:
        if client.robot.is_connected:
            client.stop()


def test_control_loop_observation_uses_next_action_timestep(robot_client, monkeypatch):
    """Observation timesteps should point to the next action to execute."""
    captured = {}

    def _fake_send_observation(obs):
        captured["obs"] = obs
        return True

    monkeypatch.setattr(robot_client, "send_observation", _fake_send_observation)
    robot_client.latest_action = 5
    robot_client.action_queue = Queue()

    raw_observation = robot_client.control_loop_observation(task="dummy")

    assert raw_observation is not None
    assert captured["obs"].get_timestep() == 6
